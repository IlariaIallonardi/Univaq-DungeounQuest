...
public class NPCServiceImpl implements PersonaIncontrataService {
    ...
    public String parla(Personaggio personaggio, NPC npc) {
        ...
        if (corretta) {
            ...
            System.out.println("L’NPC ti dona: " + tesoro.getNome());
            Zaino zaino= personaggio.getZaino();
            zaino.aggiungiOggettoAZaino( tesoro);
        } else {
            System.out.println("\n Risposta errata. L’NPC non ti dona nulla.");
        }
...
public void donaTesoro(NPC npc, Personaggio personaggio, Oggetto o) {
    ...
    // print iniziali per debug identità
...
Oggetto dono = npc.daOggetto(o);

System.out.println("DEBUG donaTesoro: dono restituito = " + dono + " | identity=" + (dono == null ? "null" : System.identityHashCode(dono)) + " classe=" + (dono == null ? "null" : dono.getClass().getName()));
   // Oggetto dono = npc.daOggetto(o);
    System.out.println("DEBUG dono = " + dono + " | classe = " + (dono == null ? "null" : dono.getClass().getName()));
    int saldoPrima = personaggio.getPortafoglioPersonaggio();
    System.out.println("Saldo prima: " + saldoPrima);

    if (dono instanceof Tesoro tesoro) {
        // usa eseguiEffetto come fa il raccogliereOggetto
        boolean applicato = tesoro.eseguiEffetto(personaggio);
        if (applicato) {
            System.out.println("L’NPC ti dona: " + tesoro.getNome());
            System.out.println("Guadagno: " + tesoro.getValore() + " monete. Saldo dopo: " + personaggio.getPortafoglioPersonaggio());
            // rimuovi il dono dalla lista dell'NPC se presente
...
public class NPCServiceImpl implements PersonaIncontrataService {
    ...
    private void popolaArticoliVenditore(NPC venditore) {
        ...
        if (venditore.getArticoli().isEmpty()) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                Oggetto o;
                switch (tipo) {
                    case 0 -> o = new PozioneServiceImpl().creaOggettoCasuale();
                    case 1 -> o = new ArmaServiceImpl().creaOggettoCasuale();
                    default -> o = new ArmaturaServiceImpl().creaOggettoCasuale();
                }
                venditore.getArticoli().add(o);
            }
        }
   ...
   public String parla(Personaggio personaggio, NPC npc) {
    if (npc == null || personaggio == null) return null;
    if (npc.haInteragito()) {
        System.out.println("\nL'NPC " + npc.getNomeNPC() + " ti ha già parlato.");
        return null;
    }
    ...
    if (corretta) {
        ...
        if (oggetto instanceof Tesoro tesoro) {
            ...
            if (zaino != null) {
                ...
                if (!added) System.out.println("Zaino pieno: il dono non è stato aggiunto.");
            } else {
    ...
    private void vendiConVenditore(Personaggio p, NPC venditore) {
        if (p == null || venditore == null) return;
        popolaArticoliVenditore(venditore);
    ...
    public void rimuoviEventoDaStanza(Stanza stanza, Evento evento) {
        ...
        if (stanza.getListaEventiAttivi() != null) stanza.getListaEventiAttivi().remove(evento);
    }

    @Override
    public boolean attivaEvento(Personaggio personaggio, Evento e) {
        if (!(e instanceof NPC npc)) return false;
    ...
    @Override
    public void eseguiEventiInStanza(Personaggio personaggio, Stanza stanza) {
        if (stanza == null || stanza.getListaEventiAttivi() == null) return;
