...
public boolean attivaEvento(Personaggio personaggio, Evento e) {
     ...
     *
     * Flusso: 1. Verifica che `e` sia una `Trappola`. 2. Chiama
     * `trappola.checkDiDisinnesco(personaggio)`. 3. Se il disinnesco fallisce:
     * - prende l'`Effetto` definito nella trappola (se presente) altrimenti
     * sceglie un effetto di fallback casuale; - invoca
     * `scattaTrappola(trappola, personaggio, effetto)` per applicare
     * stato/danno/durata al personaggio; - rimuove l'evento dalla stanza (se
     * presente) e ritorna true (turno consumato). 4. Se il disinnesco riesce:
     * rimuove l'evento e ritorna false.
     */
    @Override
    public boolean attivaEvento(Personaggio personaggio, Evento e) {
...
public void rimuoviEventoDaStanza(Stanza stanza, Evento evento) {
    ...
    public Evento aggiungiEventoCasuale() {
        ...
        String descrizione
                = switch (tipoEffetto) {
            case CONGELAMENTO ->
                "Trappola di congelamento";
            case AVVELENAMENTO ->
                "Trappola di avvelenamento";
            case STORDIMENTO ->
                "Trappola di stordimento";
            case IMMOBILIZZATO ->
                "Trappola di immobilizzazione";
            case SALTA_TURNO ->
